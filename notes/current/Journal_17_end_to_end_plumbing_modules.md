All stages were created as module but also as a standalone callable *script*.

Before starting a clean targeted refactor to move any specific config to their own file, so editing a specific value for a specific part of the stage is quicker to locate and execute.
...
The runner currently expects `.wav` files. 
> `.mp3 ` might be added later because it reduces the overall inference a bit, and file are of course smaller

Non-`.wav` inputs are skipped and moved to:
...
Purpose: 
- run pyannote diarization to identify speaker segments.
- save embeddings of every detected segments.
...
Purpose: compute low-level audio metrics using Librosa, used later for quality checks.
...
```
> Surely should be exported to a different format, the JSON is often ~ 90mb. 
...
> This is needs to not embeds every segments flagged with very suspicious indicator such as *information_byte_rate_too_high, high_compression_ratio ...*
